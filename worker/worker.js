      ...
      if (request.method === 'POST') {
        ...
        if (action === 'save') {
          ...
          await env.DB.prepare(`
            INSERT INTO clinic_days
              (key, clinic, date, date_obj, hours, t6, t3, cons, bonus, indem, gdc, start_time, end_time,
               items_json, lab_cost, dd_amount, total)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(key) DO UPDATE SET
              ...
              items_json=excluded.items_json, lab_cost=excluded.lab_cost, dd_amount=excluded.dd_amount,
          ...
          `).bind(
            ...
            r.start || '', r.end || '',
            JSON.stringify(r.items || []), r.labCost || 0, r.ddAmount || 0,
            r.total
          ).run();
